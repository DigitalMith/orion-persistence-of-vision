...
# ------------------------------
# Ensure CLI root and helpers are visible
# ------------------------------
...
try:
    ...
    from cli_helpers.ltm_helpers import embed_and_store  # ensures model loads if not already
...
def main():
    ...
    p_load.add_argument("--yaml", required=True, help="Path to persona YAML file")
    p_load.add_argument("--db", default="user_data/chroma_db", help="Chroma DB path")
    p_load.add_argument("--replace", action="store_true", help="Replace entries with same source")
    p_load.set_defaults(func=run_persona_load)

    # === persona-seed ===
    p_seed = subparsers.add_parser(
        "persona-seed",
        help="Seed persona memory from persona.yaml or .jsonl into ChromaDB"
    )
    p_seed.add_argument(
        "--file", type=str, required=True,
        help="Path to the persona.yaml or .jsonl file"
    )
    p_seed.add_argument(
        "--replace", action="store_true",
        help="Replace existing entries instead of appending (default: append)"
    )
    p_seed.add_argument(
        "--db", type=str, default=None,
        help="Override the default ChromaDB path (optional)"
    )
    ...
    p_check = subparsers.add_parser(
        "persona-check",
        help="Inspect active persona entries in ChromaDB"
    )
    ...
    # --- ingest-ltm
    ingest = subparsers.add_parser("ingest-ltm", help="Ingest staged logs with timestamp metadata")
    ingest.add_argument("--staged", default="cli/data/logs/LTM_Staged", help="Directory of .jsonl files")
    ingest.add_argument("--logs", default="cli/data/logs", help="Directory of original log files with metadata")
    ingest.add_argument("--collection", default="orion_episodic_sent_ltm", help="Chroma collection name")
    ingest.set_defaults(func=run_ingest_ltm)

    # --- ltm-query
    q = subparsers.add_parser("ltm-query", help="Query LTM using string input")
    q.add_argument("query", help="Search string")
    ...
    dt.add_argument("topic", help="Topic name to delete")
    dt.set_defaults(func=lambda args: delete_by_topic(args.topic))

    # --- summary
    sm = subparsers.add_parser("summary", help="Print memory/persona summary stats")
    sm.set_defaults(func=lambda args: summary_helper.print_summary())

    # --- web-search
    if web_search:
        ws = subparsers.add_parser("web-search", help="Search the web (and optionally save)")
        ws.add_argument("query", type=str, help="Search query")
        ws.add_argument("--save", action="store_true", help="Save result to LTM")
