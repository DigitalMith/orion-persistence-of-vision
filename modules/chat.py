...
def _tg():
    # Lazy import to break circular import at module load time
...
def generate_chat_prompt(user_input, state, **kwargs):
    ...
    if state["mode"] == "instruct":
        ...
        if state["context"].strip() != "" or state["user_bio"].strip() != "":
            context = replace_character_names(
                state["context"], state["name1"], state["name2"]
            )
    ...
    for i, entry in enumerate(reversed(history)):
        ...
        if assistant_msg:
            ...
            if (
                "<|channel|>analysis<|message|>" in assistant_msg
                or "<|channel|>final<|message|>" in assistant_msg
            ):
                thinking_content = ""
                final_content = ""
                ...
                if "<|channel|>analysis<|message|>" in assistant_msg:
                    ...
                    if len(parts) > 1:
                        ...
                        if analysis_end_tag in potential_content:
                            thinking_content = potential_content.split(
                                analysis_end_tag, 1
                            )[0].strip()
                        else:
                ...
                if final_tag_to_find in assistant_msg:
                    ...
                    if len(parts) > 1:
                        ...
                        if final_end_tag in potential_content:
                            final_content = potential_content.split(final_end_tag, 1)[
                                0
                            ].strip()
                        else:
            ...
            # Handle Seed-OSS
                ...
                if "<seed:think>" in assistant_msg:
                    ...
                    if len(parts) > 1:
                        ...
                        if "</seed:think>" in potential_content:
                            ...
                            final_content = (
                                parts[0]
                                + potential_content.split("</seed:think>", 1)[1]
                            )
                        else:
                            thinking_content = potential_content.strip()
                ...
                messages.insert(
                    insert_pos, {"role": "assistant", "content": assistant_msg}
                )
            ...
            if user_key in metadata and "attachments" in metadata[user_key]:
                ...
                for attachment in metadata[user_key]["attachments"]:
                    if attachment.get("type") == "image":
                        ...
                        if attachment.get("type") == "text/html" and attachment.get(
                            "url"
                        ):
                            attachments_text += f"\nName: {filename}\nURL: {attachment['url']}\nContents:\n\n=====\n{content}\n=====\n\n"
                        else:
    ...
    if not (impersonate or _continue):
        ...
        if len(history_data.get("metadata", {})) > 0:
            ...
            has_attachments = (
                user_key in metadata and "attachments" in metadata[user_key]
            )
    ...
    def make_prompt(messages):
        ...
        if _continue:
            if state["mode"] == "chat-instruct":
                ...
                messages.append(
                    {"role": "assistant", "content": "this will get deleted"}
                )
        ...
        if state["mode"] == "chat-instruct":
            ...
            prompt = instruct_renderer(
                messages=outer_messages, add_generation_prompt=not _continue
            )
        ...
        if _continue:
            ...
            partial_thought = last_message.get("thinking", "") or last_message.get(
                "reasoning_content", ""
            )
        ...
        if (
            state["mode"] in ["chat", "chat-instruct"]
            and not impersonate
            and not _continue
        ):
    ...
    if shared.tokenizer is not None:
        max_length = _tg().get_max_prompt_length(state)
        encoded_length = _tg().get_encoded_length(prompt)
        while len(messages) > 0 and encoded_length > max_length:
            ...
            # Resort to truncating the user input
                ...
                if encoded_length > max_length:
                    logger.error(
                        f"Failed to build the chat prompt. The input is too long for the available context length.\n\nTruncation length: {state['truncation_length']}\nmax_new_tokens: {state['max_new_tokens']} (is it too high?)\nAvailable context length: {max_length}\n"
                    )
                    raise ValueError
                else:
                    # Calculate token counts for the log message
                    original_user_tokens = _tg().get_encoded_length(user_message)
                    truncated_user_tokens = _tg().get_encoded_length(
                        user_message[:left]
                    )
...
def count_prompt_tokens(text_input, state):
    ...
    try:
        ...
        current_tokens = _tg().get_encoded_length(prompt)
...
def get_stopping_strings(state):
    ...
    for renderer in renderers:
        ...
        first_assistant_end = prompt.find("first assistant message") + len(
            "first assistant message"
        )
        second_user_start = prompt.find("second user message")
        second_assistant_end = prompt.find("second assistant message") + len(
    ...
    result = [
        ...
        if not any(
            item.startswith(other) and item != other for other in stopping_strings
        )
    ]
    ...
    # Handle GPT-OSS as a special case
    if (
        "<|channel|>final<|message|>" in state["instruction_template_str"]
        and "<|end|>" in result
    ):
        result.remove("<|end|>")
...
def add_message_version(history, role, row_idx, is_current=True):
    ...
    if is_current:
        ...
        history["metadata"][key]["current_version_index"] = (
            len(history["metadata"][key]["versions"]) - 1
        )
        ...
        if file_extension in [".jpg", ".jpeg", ".png", ".webp", ".bmp", ".gif"]:
            ...
            image_id = (
                len(
                    [
                        att
                        for att in history["metadata"][key]["attachments"]
...
def generate_search_query(user_message, state):
    ...
    query = ""
    for reply in _tg().generate_reply(
        formatted_prompt, search_state, stopping_strings=[], is_chat=True
    ):
        query = reply
...
def chatbot_wrapper(
    text, state, regenerate=False, _continue=False, loading_message=True, for_ui=False
):
    # Handle dict format with text and files
    files = []
    ...
    if not (regenerate or _continue):
        ...
        update_message_metadata(
            output["metadata"], "user", row_idx, timestamp=get_current_timestamp()
        )
        ...
        if loading_message:
            yield {
                ...
                + [[output["visible"][-1][0], shared.processing_message]],
        ...
        if regenerate:
            ...
            output["metadata"][key]["current_version_index"] = (
                len(output["metadata"][key]["versions"]) - 1
            )
    ...
    if "metadata" in output:
        for i in range(len(output["internal"])):
            user_key = f"user_{i}"
            if (
                user_key in output["metadata"]
                and "attachments" in output["metadata"][user_key]
            ):
    ...
    for j, reply in enumerate(
        ...
        if shared.stop_everything:
            output["visible"][-1][1] = apply_extensions(
                "output", output["visible"][-1][1], state, is_chat=True
            )
            yield output
            return
    ...
    if _continue:
        ...
        if state["mode"] in ["chat", "chat-instruct"]:
            full_visible = re.sub(
                "(<USER>|<user>|{{user}})", state["name1"], full_internal
            )
        else:
        ...
        output["visible"][-1][1] = apply_extensions(
            "output", full_visible, state, is_chat=True
        )
    else:
        output["visible"][-1][1] = apply_extensions(
            "output", output["visible"][-1][1], state, is_chat=True
...
def impersonate_wrapper(textbox, state):
    ...
    reply = None
    for reply in _tg().generate_reply(
        prompt + text, state, stopping_strings=stopping_strings, is_chat=True
    ):
...
def character_is_loaded(state, raise_exception=False):
    if state["mode"] in ["chat", "chat-instruct"] and state["name2"] == "":
        logger.error(
            "It looks like no character is loaded. Please load one under Parameters > Character."
        )
        if raise_exception:
            raise ValueError
...
def generate_chat_reply_wrapper(text, state, regenerate=False, _continue=False):
    ...
    for i, history in enumerate(
        generate_chat_reply(
            text, state, regenerate, _continue, loading_message=True, for_ui=True
        )
    ):
        ...
        if i == 0 or (current_time - last_save_time) >= save_interval:
            save_history(
                history, state["unique_id"], state["character_menu"], state["mode"]
            )
...
def send_dummy_message(text, state):
    ...
    update_message_metadata(
        history["metadata"], "user", row_idx, timestamp=get_current_timestamp()
    )
...
def send_dummy_reply(text, state):
    ...
    update_message_metadata(
        history["metadata"], "assistant", row_idx, timestamp=get_current_timestamp()
    )
...
def redraw_html(history, name1, name2, mode, style, character, reset_cache=False):
    return chat_html_wrapper(
        history, name1, name2, mode, style, character, reset_cache=reset_cache
    )
...
def start_new_chat(state):
    ...
    if mode != "instruct":
        greeting = replace_character_names(
            state["greeting"], state["name1"], state["name2"]
        )
        if greeting != "":
            ...
            history["visible"] += [
                [
                    "",
                    apply_extensions(
                        "output", html.escape(greeting), state, is_chat=True
                    ),
                ]
            ]
...
def rename_history(old_id, new_id, character, mode):
    ...
    if new_p.parent != old_p.parent:
        ...
        logger.error(
            f'The new path already exists and will not be overwritten: "{new_p}".'
        )
    else:
    ...
    for i, path in enumerate(histories):
        ...
        if re.match(r"^[0-9]{8}-[0-9]{2}-[0-9]{2}-[0-9]{2}$", filename):
            ...
            if data and "visible" in data and len(data["visible"]) > 0:
                if (
                    len(data["internal"]) > 0
                    and data["internal"][0][0] == "<|BEGIN-VISIBLE-CHAT|>"
                ):
...
def load_history_after_deletion(state, idx):
    ...
    if len(histories) > 0:
        history = load_history(
            histories[idx][1], state["character_menu"], state["mode"]
        )
    else:
...
def load_history(unique_id, character, mode):
    ...
    if "metadata" not in history:
        ...
        for i, (user_msg, asst_msg) in enumerate(history["internal"]):
            ...
            if asst_msg:
                update_message_metadata(
                    history["metadata"], "assistant", i, timestamp=""
                )
    ...
    try:
        ...
        if "metadata" not in history:
            ...
            for i, (user_msg, asst_msg) in enumerate(history["internal"]):
                if user_msg and user_msg != "<|BEGIN-VISIBLE-CHAT|>":
                    update_message_metadata(
                        history["metadata"], "user", i, timestamp=""
                    )
                if asst_msg:
                    update_message_metadata(
                        history["metadata"], "assistant", i, timestamp=""
...
def generate_pfp_cache(character):
    ...
    for path in [
        Path(f"user_data/characters/{character}.{extension}")
...
def load_character(character, name1, name2):
    ...
    if filepath is None or not filepath.exists():
        logger.error(
            f'Could not find the character "{character}" inside user_data/characters. No character has been loaded.'
        )
        raise ValueError
    ...
    for path in [
        Path(f"{cache_folder}/pfp_character.png"),
        Path(f"{cache_folder}/pfp_character_thumb.png"),
    ]:
        if path.exists():
            path.unlink()
    ...
    if state["character_menu"] and state["character_menu"] != "None":
        try:
            name1, name2, picture, greeting, context = load_character(
                state["character_menu"], state["name1"], state["name2"]
            )

            state["name2"] = name2
            state["greeting"] = greeting
            state["context"] = context
            state["character_picture"] = (
                picture  # This triggers cache update via generate_pfp_cache
            )
...
def load_instruction_template(template):
    ...
    for filepath in [
        Path(f"user_data/instruction-templates/{template}.yaml"),
        Path("user_data/instruction-templates/Alpaca.yaml"),
    ]:
        if filepath.exists():
            break
    else:
...
def upload_character(file, img_path, tavern=False):
    ...
    if "char_name" in data:
        ...
        name = data["name"]
        yaml_data = generate_character_yaml(
            data["name"], data["greeting"], data["context"]
        )
    ...
    with open(
        Path(f"user_data/characters/{outfile_name}.yaml"), "w", encoding="utf-8"
    ) as f:
        f.write(yaml_data)
...
def check_tavern_character(img_path):
    ...
    if img is None:
        return (
            "Invalid or disallowed image file.",
            None,
            None,
...
"""
    ...
    pre_assistant = (
        "<|bot|>"
        + params["turn_template"].split("<|bot-message|>")[0].split("<|bot|>")[1]
    )
    ...
    if "system_message" in params:
        result = result.replace(
            "<|SYSTEM-MESSAGE|>", preprocess(params["system_message"])
        )
    else:
...
def handle_branch_chat_click(state):
    ...
    if branch_from_index == -1:
        ...
        if "metadata" in history:
            history["metadata"] = {
                k: v
                for k, v in history["metadata"].items()
                if int(k.split("_")[-1]) <= branch_from_index
            }
...
def handle_edit_message_click(state):
    ...
    if (
        "versions" not in history["metadata"][key]
        or not history["metadata"][key]["versions"]
    ):
        original_content = history["internal"][message_index][role_idx]
        original_visible = history["visible"][message_index][role_idx]
        original_timestamp = history["metadata"][key].get(
            "timestamp", get_current_timestamp()
        )
    ...
    history["internal"][message_index][role_idx] = apply_extensions(
        "input", new_text, state, is_chat=True
    )
...
def handle_navigate_version_click(state):
    ...
    key = f"{role}_{message_index}"
    if (
        "metadata" not in history
        or key not in history["metadata"]
        or "versions" not in history["metadata"][key]
    ):
        html = redraw_html(
            history,
            state["name1"],
    ...
    current_idx = metadata.get(
        "current_version_index", len(versions) - 1 if versions else 0
    )
    ...
    )  # 0 for user content, 1 for assistant content in the pair
    version_to_load = versions[new_idx]
    history["internal"][message_index][msg_content_idx] = version_to_load["content"]
    history["visible"][message_index][msg_content_idx] = version_to_load[
        "visible_content"
    ]
    metadata["current_version_index"] = new_idx
    update_message_metadata(
        history["metadata"], role, message_index, timestamp=version_to_load["timestamp"]
    )
...
def handle_rename_chat_confirm(rename_to, state):
    rename_history(
        state["unique_id"], rename_to, state["character_menu"], state["mode"]
    )
    histories = find_all_histories_with_first_prompts(state)
...
def handle_character_menu_change(state):
    ...
    if len(histories) > 0:
        past_chats_update = gr.update(
            choices=histories, value=loaded_unique_id or histories[0][1]
        )
    else:
        past_chats_update = gr.update(choices=histories)
...
def handle_mode_change(state):
    ...
    if (
        state["mode"] in ["chat", "chat-instruct"]
        and state["character_menu"]
        and state["character_menu"] != "None"
    ):
