...
def _generate_reply(
    ...
    for reply in generate_func(
        question, original_question, state, stopping_strings, is_chat=is_chat
    ):
...
def encode(prompt, add_special_tokens=True, add_bos_token=True, truncation_length=None):
    ...
    # All other model types
        ...
        if shared.model.__class__.__name__ in [
            ...
            if shared.model.__class__.__name__ not in [
                "Exllamav2Model",
                "Exllamav3Model",
            ]:
                input_ids = np.array(input_ids).reshape(1, len(input_ids))
            ...
            if (
                hasattr(shared.tokenizer, "bos_token_id")
        ...
        if (
            ...
            in ["Exllamav2Model", "Exllamav3Model", "TensorRTLLMModel"]
            or shared.args.cpu
        ):
            return input_ids
        else:
...
def generate_reply_wrapper(question, state, stopping_strings=None):
    ...
    for reply in generate_reply(
        question, state, stopping_strings, is_chat=False, escape_html=True, for_ui=True
    ):
...
def get_reply_from_output_ids(output_ids, state=None, starting_from=0):
    ...
    reply = decode(
        output_ids[starting_from:], state["skip_special_tokens"] if state else True
    )

    # Handle tokenizers that do not add the leading space for the first token
    if (
        hasattr(shared.tokenizer, "convert_ids_to_tokens")
        and len(output_ids) > starting_from
    ) and not reply.startswith(" "):
        first_token = shared.tokenizer.convert_ids_to_tokens(
            int(output_ids[starting_from])
        )
...
def generate_reply_HF(
    ...
    from modules.transformers_loader import Stream, _StopEverythingStoppingCriteria
    ...
    if state["auto_max_new_tokens"]:
        generate_params["max_new_tokens"] = (
            state["truncation_length"] - input_ids.shape[-1]
        )
        ...
        # This is based on the trick of using 'stopping_criteria' to create an iterator.
            ...
            with generate_with_streaming(**generate_params) as generator:
                ...
                for output in generator:
                    ...
                    new_content = get_reply_from_output_ids(
                        output, state, starting_from=starting_from
                    )
