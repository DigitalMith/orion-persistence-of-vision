...
with open(
    Path(__file__).resolve().parent / "../css/NotoSans/stylesheet.css",
    "r",
    encoding="utf-8",
) as f:
    css = f.read()
with open(
    Path(__file__).resolve().parent / "../css/main.css", "r", encoding="utf-8"
) as f:
    css += f.read()
with open(
    Path(__file__).resolve().parent / "../css/katex/katex.min.css",
    "r",
    encoding="utf-8",
) as f:
    css += f.read()
with open(
    Path(__file__).resolve().parent / "../css/highlightjs/highlightjs-copy.min.css",
    "r",
    encoding="utf-8",
) as f:
    css += f.read()
with open(
    Path(__file__).resolve().parent / "../js/main.js", "r", encoding="utf-8"
) as f:
    js = f.read()
with open(
    Path(__file__).resolve().parent / "../js/global_scope_js.js", "r", encoding="utf-8"
) as f:
    global_scope_js = f.read()
with open(
    Path(__file__).resolve().parent / "../js/save_files.js", "r", encoding="utf-8"
) as f:
    save_files_js = f.read()
with open(
    Path(__file__).resolve().parent / "../js/switch_tabs.js", "r", encoding="utf-8"
) as f:
    switch_tabs_js = f.read()
with open(
    Path(__file__).resolve().parent / "../js/show_controls.js", "r", encoding="utf-8"
) as f:
    show_controls_js = f.read()
with open(
    Path(__file__).resolve().parent / "../js/update_big_picture.js",
    "r",
    encoding="utf-8",
) as f:
    update_big_picture_js = f.read()
with open(
    Path(__file__).resolve().parent / "../js/dark_theme.js", "r", encoding="utf-8"
) as f:
...
def gather_interface_values(*args):
    ...
    if (
        ...
        output["history"] = _load_history(
            output["unique_id"], output["character_menu"], output["mode"]
        )
...
def save_settings(
    state, preset, extensions_list, show_controls, theme_state, manual_save=False
):
    ...
    output["prompt-notebook"] = (
        state["prompt_menu-default"]
        if state["show_two_notebook_columns"]
        else state["prompt_menu-notebook"]
    )
            ...
            if extension:
                ...
                if hasattr(extension, "params"):
                    ...
                    for param in params:
                        ...
                        if (
                            param not in shared.default_settings
                            or params[param] != shared.default_settings[param]
                        ):
        ...
        if settings_path.exists():
            try:
                ...
                if "default_extensions" in existing_settings:
                    output["default_extensions"] = existing_settings[
                        "default_extensions"
                    ]
                ...
                for key, value in existing_settings.items():
                    if any(
                        key.startswith(f"{ext_name}-")
                        for ext_name in extensions_module.available_extensions
                    ):
                        output[key] = value
            except Exception:
    ...
    for key in list(output.keys()):
        if (
            key in shared.default_settings
            and output[key] == shared.default_settings[key]
        ):
            output.pop(key)
...
def store_current_state_and_debounce(
    interface_state, preset, extensions, show_controls, theme_state
):
    ...
    for element_name in change_elements:
        if element_name in shared.gradio:
            shared.gradio[element_name].change(
                ...
                store_current_state_and_debounce,
                gradio(
                    "interface_state",
                    "preset_menu",
                    "extensions_menu",
...
def create_refresh_button(
    refresh_component, refresh_method, refreshed_args, elem_class, interactive=True
):
    """
    ...
    refresh_button = gr.Button(
        refresh_symbol, elem_classes=elem_class, interactive=interactive
    )
    refresh_button.click(
        fn=lambda: {
            k: tuple(v) if type(k) is list else v for k, v in refresh().items()
        },
        inputs=[],
