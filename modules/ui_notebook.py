...
def create_ui():
    ...
    with gr.Row(
        visible=not shared.settings["show_two_notebook_columns"]
    ) as shared.gradio["notebook-tab"]:
        shared.gradio["last_input-notebook"] = gr.State("")
        with gr.Row():
            with gr.Column(scale=4):
                with gr.Tab("Raw"):
                    with gr.Row():
                        shared.gradio["textbox-notebook"] = gr.Textbox(
                            label="",
                            value="",
                            lines=27,
                            elem_id="textbox-notebook",
                            elem_classes=["textbox", "add_scrollbar"],
                        )
                        shared.gradio["token-counter-notebook"] = gr.HTML(
                            value="<span>0</span>", elem_id="notebook-token-counter"
                        )
                ...
                with gr.Tab("Logits"):
                    with gr.Row():
                        with gr.Column(scale=10):
                            shared.gradio["get_logits-notebook"] = gr.Button(
                                "Get next token probabilities"
                            )
                        with gr.Column(scale=1):
                            shared.gradio["use_samplers-notebook"] = gr.Checkbox(
                                label="Use samplers",
                                value=True,
                    ...
                    with gr.Row():
                        shared.gradio["logits-notebook"] = gr.Textbox(
                            lines=23,
                            label="Output",
                            elem_classes=["textbox_logits_notebook", "add_scrollbar"],
                        )
                        shared.gradio["logits-notebook-previous"] = gr.Textbox(
                            lines=23,
                ...
                with gr.Tab("Tokens"):
                    shared.gradio["get_tokens-notebook"] = gr.Button(
                        "Get token IDs for the input"
                    )
                    shared.gradio["tokens-notebook"] = gr.Textbox(
                        lines=23,
                        label="Tokens",
                        elem_classes=[
                            "textbox_logits_notebook",
                            "add_scrollbar",
                            "monospace",
                        ],
                    )
                ...
                with gr.Row():
                    shared.gradio["Undo"] = gr.Button(
                        "Undo", elem_classes="small-button"
                    )
                    shared.gradio["Regenerate-notebook"] = gr.Button(
                        "Regenerate", elem_classes="small-button"
                    )
                    shared.gradio["Stop-notebook"] = gr.Button(
                        "Stop",
                        visible=False,
                        elem_classes="small-button",
                        elem_id="stop",
                    )
                    shared.gradio["Generate-notebook"] = gr.Button(
            ...
            with gr.Column(scale=1):
                ...
                with gr.Row():
                    shared.gradio["prompt_menu-notebook"] = gr.Dropdown(
                        choices=utils.get_available_prompts(),
                        value=shared.settings["prompt-notebook"],
                        label="Prompt",
                ...
                with gr.Row():
                    ui.create_refresh_button(
                        shared.gradio["prompt_menu-notebook"],
                        lambda: None,
                        lambda: {"choices": utils.get_available_prompts()},
                        ["refresh-button"],
                        interactive=not mu,
                    )
                    shared.gradio["new_prompt-notebook"] = gr.Button(
                        "New", elem_classes=["refresh-button"], interactive=not mu
                    )
                    shared.gradio["rename_prompt-notebook"] = gr.Button(
                        "Rename", elem_classes=["refresh-button"], interactive=not mu
                    )
                    shared.gradio["delete_prompt-notebook"] = gr.Button(
                        "🗑️", elem_classes=["refresh-button"], interactive=not mu
                    )
                    shared.gradio["delete_prompt-confirm-notebook"] = gr.Button(
                        "Confirm",
                        variant="stop",
                        elem_classes=["refresh-button"],
                        visible=False,
                    )
                    shared.gradio["delete_prompt-cancel-notebook"] = gr.Button(
                        "Cancel", elem_classes=["refresh-button"], visible=False
                    )

                with gr.Row(visible=False) as shared.gradio["rename-row-notebook"]:
                    shared.gradio["rename_prompt_to-notebook"] = gr.Textbox(
                        label="New name", elem_classes=["no-background"]
                    )
                    shared.gradio["rename_prompt-cancel-notebook"] = gr.Button(
                        "Cancel", elem_classes=["refresh-button"]
                    )
                    shared.gradio["rename_prompt-confirm-notebook"] = gr.Button(
                        "Confirm", elem_classes=["refresh-button"], variant="primary"
                    )


def create_event_handlers():
    shared.gradio["Generate-notebook"].click(
        lambda x: x, gradio("textbox-notebook"), gradio("last_input-notebook")
    ).then(
        ...
        None,
        gradio("Stop-notebook", "Generate-notebook"),
    ).then(
        generate_and_save_wrapper_notebook,
        gradio("textbox-notebook", "interface_state", "prompt_menu-notebook"),
        gradio(outputs),
        show_progress=False,
    ).then(
        lambda state, text: state.update({"textbox-notebook": text}),
        gradio("interface_state", "textbox-notebook"),
        None,
    ).then(
    ...
    shared.gradio["Regenerate-notebook"].click(
        lambda x: x,
        gradio("last_input-notebook"),
        gradio("textbox-notebook"),
        show_progress=False,
    ).then(
        ui.gather_interface_values,
        gradio(shared.input_elements),
    ...
    shared.gradio["markdown_render-notebook"].click(
        lambda x: x,
        gradio("textbox-notebook"),
        gradio("markdown-notebook"),
        queue=False,
    )
    shared.gradio["Stop-notebook"].click(stop_everything_event, None, None, queue=False)
    shared.gradio["prompt_menu-notebook"].change(
        load_prompt,
        gradio("prompt_menu-notebook"),
        gradio("textbox-notebook"),
        show_progress=False,
    )
    shared.gradio["new_prompt-notebook"].click(
        handle_new_prompt, None, gradio("prompt_menu-notebook"), show_progress=False
    )
    ...
    shared.gradio["delete_prompt-notebook"].click(
        ...
        None,
        gradio(
            "delete_prompt-notebook",
            "delete_prompt-cancel-notebook",
            "delete_prompt-confirm-notebook",
        ),
        show_progress=False,
    )
    ...
    shared.gradio["rename_prompt-notebook"].click(
        ...
        gradio("prompt_menu-notebook"),
        gradio(
            "rename_prompt_to-notebook", "rename_prompt-notebook", "rename-row-notebook"
        ),
        show_progress=False,
    )
    ...
    shared.gradio["textbox-notebook"].input(
        lambda x: f"<span>{count_tokens(x)}</span>",
        gradio("textbox-notebook"),
        gradio("token-counter-notebook"),
        show_progress=False,
    )
    ...
    shared.gradio["get_logits-notebook"].click(
        ...
        gradio("logits-notebook", "logits-notebook-previous"),
        show_progress=False,
    )

    shared.gradio["get_tokens-notebook"].click(
        get_token_ids,
        gradio("textbox-notebook"),
        gradio("tokens-notebook"),
        show_progress=False,
    )
...
def generate_and_save_wrapper_notebook(textbox_content, interface_state, prompt_name):
    ...
    for i, (output, html_output) in enumerate(
        generate_reply_wrapper(textbox_content, interface_state)
    ):
...
def handle_delete_prompt_confirm_notebook(prompt_name):
    ...
    current_index = (
        available_prompts.index(prompt_name) if prompt_name in available_prompts else 0
    )
    ...
    if available_prompts:
        ...
        (Path("user_data/logs/notebook") / f"{new_value}.txt").write_text(
            "In this story,"
        )
