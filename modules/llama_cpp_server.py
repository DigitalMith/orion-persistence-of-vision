...
class LlamaServer:
    ...
    def prepare_payload(self, state):
        ...
        if state["custom_token_bans"]:
            to_ban = [
                [int(token_id), False]
                for token_id in state["custom_token_bans"].split(",")
            ]
    ...
    def _process_images_for_generation(self, state: dict) -> List[Any]:
        ...
        # Source 2: Chat Completions API (/v1/chat/completions)
            ...
            pil_images.extend(
                convert_openai_messages_to_images(state["history"]["messages"])
            )
    ...
    def generate_with_streaming(self, prompt, state):
        ...
        if pil_images:
            ...
            self.last_prompt_token_count = len(text_tokens) + (
                len(pil_images) * IMAGE_TOKEN_COST_ESTIMATE
            )
        else:
            # Text only case
        ...
        payload.update(
            {"n_predict": max_new_tokens, "stream": True, "cache_prompt": True}
        )
        ...
        try:
            if (
                ...
                and response.json()["error"]["type"] == "exceed_context_size_error"
            ):
                logger.error(
    ...
    def get_logits(self, prompt, state, n_probs=128, use_samplers=False):
        ...
        for retry in range(5):
            ...
            raise Exception(
                f"Unexpected response format: 'completion_probabilities' not found in {result}"
            )
    ...
    def _start_server(self):
        ...
        if (
            ...
            and shared.args.cache_type in llamacpp_valid_cache_types
        ):
            cmd += [
                "--cache-type-k",
        ...
        if os.name == "posix":
            ...
            if current_path:
                env["LD_LIBRARY_PATH"] = (
                    f"{current_path}:{os.path.dirname(self.server_path)}"
                )
            else:
        ...
        threading.Thread(
            target=filter_stderr_with_progress, args=(self.process.stderr,), daemon=True
        ).start()
                ...
                try:
                    ...
                    if line:  # Process non-empty lines
                        ...
                        if match:
                            ...
                            print(
                                display_line, end=end_char, file=sys.stderr, flush=True
                            )
                            last_was_progress = progress < 1.0

                        # skip noise lines
                        elif not (
                            line.startswith(("srv ", "slot "))
                            or "log_server_r: request: GET /health" in line
                        ):
