...
with open(
    Path(__file__).resolve().parent / "../css/html_readable_style.css",
    "r",
    encoding="utf-8",
) as f:
    readable_css = f.read()
with open(
    Path(__file__).resolve().parent / "../css/html_instruct_style.css",
    "r",
    encoding="utf-8",
) as f:
    instruct_css = f.read()
...
def replace_quotes(text):
    ...
    pattern = "|".join(
        f"({re.escape(open_q)})(.*?)({re.escape(close_q)})"
...
def replace_blockquote(m):
    return (
        m.group()
        .replace("\n", "\n> ")
        .replace("\\begin{blockquote}", "")
        .replace("\\end{blockquote}", "")
    )
...
def extract_thinking_block(string):
    ...
    if alt_start_pos != -1 or alt_end_pos != -1:
        ...
        if alt_end_pos == -1:
            ...
            content_start = (
                alt_content_pos + len(ALT_CONTENT_START)
                if alt_content_pos != -1
                else alt_end_pos + len(ALT_END)
            )
...
def process_markdown_content(string):
    ...
    if re.search(list_item_pattern, result):
        ...
        html_output = markdown.markdown(
            result, extensions=["fenced_code", "tables", SaneListExtension()]
        )
...
def generate_basic_html(string):
    ...
    string = (
        f'<style>{readable_css}</style><div class="readable-container">{string}</div>'
    )
    return string
...
def get_image_cache(path):
    ...
    if (path in image_cache and mtime != image_cache[path][0]) or (
        path not in image_cache
    ):
...
def format_message_timestamp(history, role, index, tooltip_include_timestamp=True):
    ...
    if (
        "metadata" in history
        and key in history["metadata"]
        and history["metadata"][key].get("timestamp")
    ):
        timestamp = history["metadata"][key]["timestamp"]
        tooltip_text = get_message_tooltip(
            history, role, index, include_timestamp=tooltip_include_timestamp
        )
...
def format_message_attachments(history, role, index):
    ...
    if (
        "metadata" in history
        and key in history["metadata"]
        and "attachments" in history["metadata"][key]
    ):
...
def get_version_navigation_html(history, i, role):
    ...
    position = (
        f'<span class="version-position">{current_idx + 1}/{len(versions)}</span>'
    )
...
def actions_html(history, i, role, info_message=""):
    ...
    if role == "assistant":
        ...
        action_buttons = f"{copy_button}" f"{edit_button}"
...
def generate_instruct_html(history, last_message_only=False):
    ...
    def create_message(role, content, raw_content):
        ...
        if timestamp:
            ...
            info_message = info_button.replace(
                'title="message"', f'title="{html.escape(tooltip_text)}"'
            )
        ...
        if last_message_only:
            converted_visible = [
                ...
                convert_to_markdown_wrapped(
                    row_visible[1],
                    message_id=i,
                    use_cache=i != len(history["visible"]) - 1,
                ),
            ]
        else:
            converted_visible = [
                convert_to_markdown_wrapped(
                    entry, message_id=i, use_cache=i != len(history["visible"]) - 1
                )
                for entry in row_visible
            ]
...
def generate_cai_chat_html(
    history, name1, name2, style, character, reset_cache=False, last_message_only=False
):
    if not last_message_only:
    ...
    def create_message(role, content, raw_content):
        ...
        timestamp = format_message_timestamp(
            history, role, i, tooltip_include_timestamp=False
        )
        attachments = format_message_attachments(history, role, i)
