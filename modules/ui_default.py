...
def create_ui():
    ...
    with gr.Row(visible=shared.settings["show_two_notebook_columns"]) as shared.gradio[
        "default-tab"
    ]:
        with gr.Row():
            with gr.Column():
                with gr.Row():
                    shared.gradio["textbox-default"] = gr.Textbox(
                        value="",
                        lines=27,
                        label="Input",
                        elem_classes=["textbox_default", "add_scrollbar"],
                    )
                    shared.gradio["token-counter-default"] = gr.HTML(
                        value="<span>0</span>", elem_id="default-token-counter"
                    )

                with gr.Row():
                    shared.gradio["Continue-default"] = gr.Button("Continue")
                    shared.gradio["Stop-default"] = gr.Button(
                        "Stop", elem_id="stop", visible=False
                    )
                    shared.gradio["Generate-default"] = gr.Button(
                        "Generate", variant="primary"
                    )

                with gr.Row():
                    shared.gradio["prompt_menu-default"] = gr.Dropdown(
                    ...
                    ui.create_refresh_button(
                        shared.gradio["prompt_menu-default"],
                        lambda: None,
                        lambda: {"choices": utils.get_available_prompts()},
                        "refresh-button",
                        interactive=not mu,
                    )
                    shared.gradio["new_prompt-default"] = gr.Button(
                        "New", elem_classes="refresh-button", interactive=not mu
                    )
                    shared.gradio["rename_prompt-default"] = gr.Button(
                        "Rename", elem_classes="refresh-button", interactive=not mu
                    )
                    shared.gradio["delete_prompt-default"] = gr.Button(
                        "🗑️", elem_classes="refresh-button", interactive=not mu
                    )

                    # Rename elements (initially hidden)
                    shared.gradio["rename_prompt_to-default"] = gr.Textbox(
                        label="New name", elem_classes=["no-background"], visible=False
                    )
                    shared.gradio["rename_prompt-cancel-default"] = gr.Button(
                        "Cancel", elem_classes=["refresh-button"], visible=False
                    )
                    shared.gradio["rename_prompt-confirm-default"] = gr.Button(
                        "Confirm",
                    ...
                    shared.gradio["delete_prompt-cancel-default"] = gr.Button(
                        "Cancel", elem_classes=["refresh-button"], visible=False
                    )
                    shared.gradio["delete_prompt-confirm-default"] = gr.Button(
                        "Confirm",
                        variant="stop",
                        elem_classes=["refresh-button"],
                        visible=False,
            ...
            with gr.Column():
                ...
                with gr.Tab("Logits"):
                    with gr.Row():
                        with gr.Column(scale=10):
                            shared.gradio["get_logits-default"] = gr.Button(
                                "Get next token probabilities"
                            )
                        with gr.Column(scale=1):
                            shared.gradio["use_samplers-default"] = gr.Checkbox(
                                label="Use samplers",
                                value=True,
                    ...
                    with gr.Row():
                        shared.gradio["logits-default"] = gr.Textbox(
                            lines=23,
                            label="Output",
                            elem_classes=["textbox_logits", "add_scrollbar"],
                        )
                        shared.gradio["logits-default-previous"] = gr.Textbox(
                            lines=23,
                ...
                with gr.Tab("Tokens"):
                    shared.gradio["get_tokens-default"] = gr.Button(
                        "Get token IDs for the input"
                    )
                    shared.gradio["tokens-default"] = gr.Textbox(
                        lines=23,
                        label="Tokens",
                        elem_classes=["textbox_logits", "add_scrollbar", "monospace"],
                    )
...
def create_event_handlers():
    shared.gradio["Generate-default"].click(
        ...
        None,
        gradio("Stop-default", "Generate-default"),
    ).then(
        generate_reply_wrapper,
        gradio("textbox-default", "interface_state"),
        gradio(outputs),
        show_progress=False,
    ).then(
        lambda state, left, right: state.update(
            {"textbox-default": left, "output_textbox": right}
        ),
        gradio("interface_state", "textbox-default", "output_textbox"),
        None,
    ).then(
    ...
    shared.gradio["Continue-default"].click(
        ...
        generate_reply_wrapper,
        gradio("output_textbox", "interface_state"),
        gradio(outputs),
        show_progress=False,
    ).then(
        lambda state, left, right: state.update(
    ...
    shared.gradio["markdown_render-default"].click(
        lambda x: x, gradio("output_textbox"), gradio("markdown-default"), queue=False
    )
    shared.gradio["prompt_menu-default"].change(
        lambda x: (load_prompt(x), ""),
        gradio("prompt_menu-default"),
        gradio("textbox-default", "output_textbox"),
        show_progress=False,
    )
    shared.gradio["new_prompt-default"].click(
        handle_new_prompt, None, gradio("prompt_menu-default"), show_progress=False
    )
    ...
    shared.gradio["delete_prompt-default"].click(
        ...
        gradio(
            ...
            "delete_prompt-cancel-default",
            "delete_prompt-confirm-default",
        ),
        show_progress=False,
    )
    ...
    shared.gradio["rename_prompt-default"].click(
        ...
        gradio("prompt_menu-default"),
        gradio(
            "rename_prompt_to-default",
            "rename_prompt-default",
            "rename_prompt-cancel-default",
            "rename_prompt-confirm-default",
        ),
        show_progress=False,
    )
    ...
    shared.gradio["textbox-default"].change(
        ...
        gradio("textbox-default"),
        gradio("token-counter-default"),
        show_progress=False,
    )
    shared.gradio["get_logits-default"].click(
        ...
        gradio("logits-default", "logits-default-previous"),
        show_progress=False,
    )

    shared.gradio["get_tokens-default"].click(
        get_token_ids,
        gradio("textbox-default"),
        gradio("tokens-default"),
        show_progress=False,
    )
