...
class Exllamav3Model:
    ...
    def from_pretrained(cls, path_to_model):
        ...
        cache = Cache(
            model, max_num_tokens=max_tokens, layer_type=layer_type, **cache_kwargs
        )
        ...
        draft_model = None
        draft_cache = None
        if shared.args.model_draft and shared.args.model_draft.lower() not in [
            "",
            "none",
        ]:
            logger.info(
                f"Loading draft model for speculative decoding: {shared.args.model_draft}"
            )

            draft_path = Path(shared.args.model_draft)
            if not draft_path.is_dir():
                draft_path = Path(f"{shared.args.model_dir}") / Path(
                    shared.args.model_draft
                )

            if not draft_path.is_dir():
                logger.warning(
                    f"Draft model not found at {draft_path}, speculative decoding disabled."
                )
            else:
                ...
                if draft_max_tokens % 256 != 0:
                    ...
                    logger.warning(
                        f"Draft model max_num_tokens must be a multiple of 256. Adjusting from {draft_max_tokens} to {adjusted_draft_tokens}"
                    )
                ...
                draft_cache = Cache(
                    draft_model,
                    max_num_tokens=draft_max_tokens,
                ...
                logger.info(
                    f"Draft model loaded successfully. Max speculative tokens: {shared.args.draft_max}"
                )
        ...
        if "vision_config" in config.config_dict:
            logger.info(
                "Vision component detected in model config. Attempting to load..."
            )
            try:
                ...
                logger.warning(
                    f"Vision model loading failed (multimodal disabled): {e}"
                )
        else:
            logger.info(
                "No vision component in model config. Skipping multimodal setup."
            )
        ...
        generator = Generator(
            ...
            num_speculative_tokens=(
                shared.args.draft_max if draft_model is not None else 0
            ),
        )
    ...
    def _process_images_for_generation(
        ...
        if "image_attachments" in state and state["image_attachments"]:
            pil_images.extend(
                convert_image_attachments_to_pil(state["image_attachments"])
            )
        ...
        try:
            ...
            if "image_embeddings" in state and state["image_embeddings"]:
                ...
                logger.info(
                    f"Processing {len(pil_images)} image(s) with ExLlamaV3 vision model"
                )
                image_embeddings = [
                    self.vision_model.get_image_embeddings(
                        tokenizer=self.tokenizer, image=img
                    )
    ...
    def generate_with_streaming(self, prompt, state):
        ...
        if shared.is_multimodal:
            ...
            prompt, image_embeddings = self._process_images_for_generation(
                prompt, state
            )
        else:
            image_embeddings = []
        ...
        if state["temperature"] == 0:
            ...
            if state["repetition_penalty"] != 1.0:
                unordered_samplers.append(
                    SS_RepP(state["repetition_penalty"], penalty_range, rep_decay)
                )
            ...
            if state["temperature_last"] and "temperature" in sampler_priority:
                sampler_priority.append(
                    sampler_priority.pop(sampler_priority.index("temperature"))
                )
    ...
    def get_logits(self, token_ids, **kwargs):
        ...
        if token_ids.shape[-1] > 1:
            ...
            self.model.forward(input_ids=prefix_ids, params=params)
        ...
        logits = self.model.forward(input_ids=last_token_ids, params=params)
