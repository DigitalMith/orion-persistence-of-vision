...
def _get_next_logits(
    prompt, state, use_samplers, previous, top_logits=25, return_dict=False
):
    ...
    if shared.model.__class__.__name__ == "LlamaServer":
        logprobs = shared.model.get_logits(
            prompt, state, n_probs=top_logits, use_samplers=use_samplers
        )
    ...
    # All other model types
        ...
        if use_samplers:
            if is_non_hf_exllamav2:
                ...
                logger.error(
                    "Sampler hijacking is not supported non-Huggingface loaders."
                )
                return (
        ...
        topk_values, topk_indices = torch.topk(
            probs, k=top_logits, largest=True, sorted=True
        )
        if hasattr(shared.tokenizer, "convert_ids_to_tokens"):
            tokens = [
                shared.tokenizer.convert_ids_to_tokens(int(i)) for i in topk_indices
            ]
        else:
