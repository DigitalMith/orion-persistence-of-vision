...
group.add_argument(
    "--loader",
    type=str,
    help="Choose the model loader manually, otherwise, it will get autodetected. Valid options: Transformers, llama.cpp, ExLlamav3_HF, ExLlamav2_HF, ExLlamav2, TensorRT-LLM.",
)
...
group.add_argument(
    ...
    default="fp16",
    metavar="N",
    help="KV cache type; valid options: llama.cpp - fp16, q8_0, q4_0; ExLlamaV2 - fp16, fp8, q8, q6, q4; ExLlamaV3 - fp16, q2 to q8 (can specify k_bits and v_bits separately, e.g. q4_q8).",
)
...
group.add_argument(
    ...
    type=int,
    default=256,
    help="Number of layers to offload to the GPU for the draft model.",
)
group.add_argument(
    "--device-draft",
    type=str,
...
group.add_argument(
    ...
    type=int,
    default=0,
    help="Number of threads to use for batches/prompt processing.",
)
group.add_argument(
    "--numa", action="store_true", help="Activate NUMA task allocation for llama.cpp."
)
group.add_argument(
    "--extra-flags",
    type=str,
...
group.add_argument(
    ...
    type=float,
    default=0,
    help="Maximum CPU memory in GiB. Use this for CPU offloading.",
)
group.add_argument(
    "--disk",
    action="store_true",
...
group.add_argument(
    "--disk-cache-dir",
    type=str,
    default="user_data/cache",
    help='Directory to save the disk cache to. Defaults to "user_data/cache".',
)
group.add_argument(
    "--load-in-8bit",
    action="store_true",
...
group.add_argument(
    "--no-cache",
    action="store_true",
    help="Set use_cache to False while generating text. This reduces VRAM usage slightly, but it comes at a performance cost.",
)
group.add_argument(
    "--trust-remote-code",
    action="store_true",
    help="Set trust_remote_code=True while loading the model. Necessary for some models.",
)
group.add_argument(
    "--force-safetensors",
    action="store_true",
    help="Set use_safetensors=True while loading the model. This prevents arbitrary code execution.",
)
group.add_argument(
    "--no_use_fast",
    action="store_true",
    help="Set use_fast=False while loading the tokenizer (it's True by default). Use this if you have any problems related to use_fast.",
)
group.add_argument(
    "--attn-implementation",
    type=str,
    ...
    help="Attention implementation. Valid options: sdpa, eager, flash_attention_2.",
)
...
group.add_argument(
    ...
    "--enable_tp",
    action="store_true",
    help="Enable Tensor Parallelism (TP) to split the model across GPUs.",
)
group.add_argument(
    "--tp-backend",
    type=str,
    default="native",
    help="The backend for tensor parallelism. Valid options: native, nccl. Default: native.",
)
...
group.add_argument(
    "--gpu-split",
    type=str,
    help="Comma-separated list of VRAM (in GB) to use per GPU device for model layers. Example: 20,7,7.",
)
group.add_argument(
    "--autosplit",
    action="store_true",
    help="Autosplit the model tensors across the available GPUs. This causes --gpu-split to be ignored.",
)
group.add_argument(
    "--cfg-cache",
    action="store_true",
    help="ExLlamav2_HF: Create an additional cache for CFG negative prompts. Necessary to use CFG with that loader.",
)
group.add_argument(
    "--no_flash_attn", action="store_true", help="Force flash-attention to not be used."
)
group.add_argument(
    "--no_xformers", action="store_true", help="Force xformers to not be used."
)
group.add_argument(
    "--no_sdpa", action="store_true", help="Force Torch SDPA to not be used."
)
group.add_argument(
    ...
    default=2,
    metavar="N",
    help="Number of experts to use for generation. Applies to MoE models like Mixtral.",
)
...
group.add_argument(
    "--cpp-runner",
    action="store_true",
    help="Use the ModelRunnerCpp runner, which is faster than the default ModelRunner but doesn't support streaming yet.",
)
...
group.add_argument(
    "--ssl-keyfile",
    type=str,
    help="The path to the SSL certificate key file.",
    default=None,
)
group.add_argument(
    "--ssl-certfile",
    type=str,
    help="The path to the SSL certificate cert file.",
    default=None,
)
group.add_argument(
    "--subpath",
...
group.add_argument(
    "--old-colors",
    action="store_true",
    help="Use the legacy Gradio colors, before the December/2024 update.",
)
group.add_argument(
    "--portable",
    action="store_true",
    help="Hide features not available in portable mode like training.",
)
...
group.add_argument(
    "--api-port", type=int, default=5000, help="The listening port for the API."
)
...
group.add_argument(
    "--api-enable-ipv6", action="store_true", help="Enable IPv6 for the API"
)
group.add_argument(
    "--api-disable-ipv4", action="store_true", help="Disable IPv4 for the API"
)
group.add_argument(
    "--nowebui",
    action="store_true",
...
settings = {
    ...
    "preset": (
        "Qwen3 - Thinking"
        if Path("user_data/presets/Qwen3 - Thinking.yaml").exists()
        else None
    ),
    "max_new_tokens": 512,
...
def do_cmd_flags_warnings():
    ...
    if "COLAB_GPU" not in os.environ and not args.nowebui:
        ...
        if any((args.listen, args.share)) and not any(
            (args.gradio_auth, args.gradio_auth_path)
        ):
            logger.warning(
