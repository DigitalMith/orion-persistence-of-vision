...
def create_ui():
    ...
    with gr.Tab("Training", elem_id="training-tab"):
        with gr.Tab("Train LoRA", elem_id="lora-train-tab"):
            ...
            with gr.Row():
                with gr.Column():
                    gr.Markdown(
                        "[Tutorial](https://github.com/oobabooga/text-generation-webui/wiki/05-%E2%80%90-Training-Tab)"
                    )
                    ...
                    with gr.Row():
                        ...
                        with gr.Column():
                            always_override = gr.Checkbox(
                                ...
                                value=False,
                                info="If the name is the same, checking will replace the existing file, and unchecking will load and continue from it (the rank must be the same).",
                                elem_classes=["no-background"],
                            )

                    with gr.Accordion(
                        label="Target Modules", open=False, elem_classes="tgw-accordion"
                    ):
                        gr.Markdown(
                        ...
                        with gr.Row():
                            with gr.Column():
                                q_proj_en = gr.Checkbox(
                                    label="Enable q_proj", value=True
                                )
                            with gr.Column():
                                v_proj_en = gr.Checkbox(
                                    label="Enable v_proj", value=True
                                )
                            with gr.Column():
                                k_proj_en = gr.Checkbox(
                                    label="Enable k_proj", value=False
                                )
                            with gr.Column():
                                o_proj_en = gr.Checkbox(
                                    label="Enable o_proj", value=False
                                )
                            with gr.Column():
                                gate_proj_en = gr.Checkbox(
                                    label="Enable gate_proj", value=False
                                )
                            with gr.Column():
                                down_proj_en = gr.Checkbox(
                                    label="Enable down_proj", value=False
                                )
                            with gr.Column():
                                up_proj_en = gr.Checkbox(
                                    label="Enable up_proj", value=False
                                )
                    ...
                    with gr.Row():
                        with gr.Column():
                            lora_rank = gr.Slider(
                                ...
                                maximum=1024,
                                step=4,
                                info="Also called dimension count. Higher values = larger file, more content control. Smaller values = smaller file, less control. Use 4 or 8 for style, 128 or 256 to teach, 1024+ for fine-detail on big data. More VRAM is needed for higher ranks.",
                            )
                            lora_alpha = gr.Slider(
                                ...
                                maximum=2048,
                                step=4,
                                info="This divided by the rank becomes the scaling of the LoRA. Higher means stronger. A good standard value is twice your Rank.",
                            )
                        ...
                        with gr.Column():
                            ...
                            learning_rate = gr.Textbox(
                                label="Learning Rate",
                                value="3e-4",
                                info="In scientific notation. 3e-4 is a good starting base point. 1e-2 is extremely high, 1e-6 is extremely low.",
                            )
                            with gr.Row():
                ...
                with gr.Column():
                    with gr.Tab(label="Formatted Dataset"):
                        with gr.Row():
                            format = gr.Dropdown(
                                choices=utils.get_datasets(
                                    "user_data/training/formats", "json"
                                ),
                                value="None",
                                label="Data Format",
                        ...
                        with gr.Row():
                            dataset = gr.Dropdown(
                                choices=utils.get_datasets(
                                    "user_data/training/datasets", "json"
                                ),
                                value="None",
                                label="Dataset",
                        ...
                        with gr.Row():
                            with gr.Column():
                                overlap_len = gr.Slider(
                                    ...
                                    value=128,
                                    step=16,
                                    info="How many tokens from the prior chunk of text to include into the next chunk. (The chunks themselves will be of a size determined by Cutoff Length). Setting overlap to exactly half the cutoff length may be ideal.",
                                )
                    ...
                    with gr.Row():
                        start_button = gr.Button(
                            "Start LoRA Training", variant="primary", interactive=not mu
                        )
        ...
        with gr.Tab("Perplexity evaluation", elem_id="evaluate-tab"):
            with gr.Row():
                with gr.Column():
                    ...
                    evaluate_text_file = gr.Dropdown(
                        ...
                        + utils.get_datasets("user_data/training/datasets", "txt")[1:],
                        value="wikitext",
                        label="Input dataset",
                        info="The raw text file on which the model will be evaluated. The first options are automatically downloaded: wikitext, ptb, and ptb_new. The next options are your local text files under user_data/training/datasets.",
                        interactive=not mu,
                    )
                    with gr.Row():
                        with gr.Column():
                    ...
                    with gr.Row():
                        start_current_evaluation = gr.Button(
                            "Evaluate loaded model", interactive=not mu
                        )
                        start_evaluation = gr.Button(
                            "Evaluate selected models", interactive=not mu
                        )
            ...
            with gr.Row():
                save_comments = gr.Button(
                    "Save comments", elem_classes="small-button", interactive=not mu
                )
                refresh_table = gr.Button(
                    "Refresh the table", elem_classes="small-button", interactive=not mu
                )
    ...
    ev_cur = start_current_evaluation.click(lambda: ["current model"], None, tmp).then(
        calculate_perplexity,
...
def do_train(
    ...
    def list_target_modules(model_id):
        ...
        target_mods = [
            f"{name}_proj" for name, enabled in available_modules.items() if enabled
        ]
        return target_mods
    ...
    if raw_text_file not in ["None", ""]:
        ...
        if fullpath.is_dir():
            ...
            with open(
                clean_path("user_data/training/datasets", f"{raw_text_file}.txt"),
                "r",
                encoding="utf-8",
            ) as file:
                raw_text = file.read().replace("\r", "")
        ...
        with open(
            clean_path("user_data/training/formats", f"{format}.json"),
            "r",
            encoding="utf-8-sig",
        ) as formatFile:
        ...
        data = load_dataset(
            "json",
            data_files=clean_path("user_data/training/datasets", f"{dataset}.json"),
        )
        ...
        if eval_dataset == "None":
            ...
            eval_data = load_dataset(
                ...
                data_files=clean_path(
                    "user_data/training/datasets", f"{eval_dataset}.json"
                ),
            )
    ...
    try:
        ...
        if (
            ...
            state_dict_peft = torch.load(
                f"{lora_file_path}/adapter_model.bin", weights_only=True
            )
    ...
    trainer = transformers.Trainer(
        ...
        args=transformers.TrainingArguments(
            ...
            eval_steps=(
                math.ceil(eval_steps / gradient_accumulation_steps)
            ...
            use_ipex=(
                True if is_torch_xpu_available() and not shared.args.cpu else False
            ),
        ),
    ...
    projections_string = ", ".join(
        [
            ...
            for projection in list_target_modules(model_id)
        ]
    )
    ...
    def log_train_dataset(trainer):
        ...
        try:
            ...
            with open(
                Path("user_data/logs/train_dataset_sample.json"), "w"
            ) as json_file:
                json.dump(decoded_entries, json_file, indent=4)

            logger.info(
                "Log file 'train_dataset_sample.json' created in the 'user_data/logs' directory."
            )
        except Exception as e:
