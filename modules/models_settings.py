...
def get_model_metadata(model):
    ...
    if "loader" not in model_settings:
        quant_method = (
            ...
            else hf_metadata.get("quantization_config", {}).get("quant_method", None)
        )
        model_settings["loader"] = infer_loader(
            model, model_settings, hf_quant_method=quant_method
        )
    ...
    if model_settings["loader"] == "llama.cpp":
        ...
        if "tokenizer.chat_template" in metadata:
            ...
            model_settings["instruction_template"] = (
                "Custom (obtained from model metadata)"
            )
...
def apply_model_settings_to_state(model, state):
    ...
    if "loader" in model_settings:
        ...
        if not (
            (loader == "ExLlamav2_HF" and state["loader"] == "ExLlamav2")
            or (loader == "ExLlamav3_HF" and state["loader"] == "ExLlamav3")
        ):
            state["loader"] = loader
...
def save_instruction_template(model, template):
    ...
    if template == "None":
        yield (
            f"Instruction template for `{model}` unset in `{p}`, as the value for template was `{template}`."
        )
    else:
...
def estimate_vram(gguf_file, gpu_layers, ctx_size, cache_type):
    ...
    vram = (
        ...
        + max(
            0.9690636483914102,
            cache_type
            - (floor(50.77817218646521 * embedding_per_context) + 9.987899908205632),
        )
    ) + 1516.522943869404
...
def update_gpu_layers_and_vram(
    loader, model, gpu_layers, ctx_size, cache_type, auto_adjust=False, for_ui=True
):
    """
    ...
    if auto_adjust:
        ...
        has_user_setting = (
            model_regex in user_config and "gpu_layers" in user_config[model_regex]
        )
        ...
        if not has_user_setting:
            ...
            return_free = (
                False if (for_ui and shared.model_name not in [None, "None"]) else True
            )
            ...
            if available_vram > 0:
                ...
                while (
                    current_layers > 0
                    and estimate_vram(model, current_layers, ctx_size, cache_type)
                    > available_vram - tolerance
                ):
                    current_layers -= 1
