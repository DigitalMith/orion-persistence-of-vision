...
class Exllamav2Model:
    ...
    def from_pretrained(self, path_to_model):
        ...
        if shared.args.model_draft and shared.args.model_draft.lower() not in [
            ...
            logger.info(
                f"Draft model loaded successfully with max_draft={shared.args.draft_max}"
            )
    ...
    def encode(self, string, **kwargs):
        ...
        return self.tokenizer.encode(
            string, add_bos=add_bos, encode_special_tokens=True, **kwargs
        )
    ...
    def generate_with_streaming(self, prompt, state):
        ...
        settings.max_temp = (
            state["dynatemp_high"] if state["dynamic_temperature"] else 0
        )
        ...
        if hasattr(self, "draft_model") and self.draft_model is not None:
            ...
            ) = self.generator.get_sd_stats()
            logger.info(
                f"Speculative decoding: accepted={accepted_draft_tokens}/{total_draft_tokens} tokens"
            )
