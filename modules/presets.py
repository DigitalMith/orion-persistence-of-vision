...
def load_preset(name, verbose=False):
    ...
    if name not in ["None", None, ""]:
        ...
        if path.exists():
            ...
            logger.error(
                f'The preset "{name}" does not exist under "{path}". Using the default parameters.'
            )

    if verbose:
        logger.info(f'"{name}" preset:')
        pprint.PrettyPrinter(indent=4, width=1, sort_dicts=False).pprint(
            remove_defaults(generate_params)
        )
...
def loader_contains(sampler):
    if (
        sampler == "dynamic_temperature"
        and "dynatemp_low" in loaders_samplers[shared.args.loader]
    ):
        return True
    else:
