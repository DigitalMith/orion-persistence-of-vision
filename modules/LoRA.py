...
def add_lora_to_model(lora_names):
    if shared.model.__class__.__name__ in [
        "Exllamav2Model",
        "Exllamav2HF",
    ] or shared.args.loader in ["ExLlamav2", "ExLlamav2_HF"]:
        ...
        for lora_name in lora_names:
            ...
            if shared.model.__class__.__name__ == "Exllamav2Model":
                ...
                lora = ExLlamaV2Lora.from_directory(
                    shared.model.ex_model, str(lora_path)
                )
...
def merge_loras():
    if (
        len(
            list(
                {
                    shared.model.peft_config[adapter].r
                    for adapter in shared.model.peft_config.keys()
                }
            )
        )
        > 1
        ...
        logger.warning(
            "The loaded LoRAs cannot be merged, as they have dissimilar ranks. Only the first one will be active."
        )
        return

    shared.model.add_weighted_adapter(
        shared.lora_names, [1] * len(shared.lora_names), "__merged"
    )
