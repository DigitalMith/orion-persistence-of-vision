...
KEEPSAKES_COLLECTION = "keepsakes"
...
def _keepsakes():
    name = _collection_name("keepsakes", "keepsake", default=KEEPSAKES_COLLECTION)
    return _get_collection(name)
...
def clear_persona_collection() -> int:
    """
    Clear ONLY the persona collection.

    Returns:
        int: number of items removed (best-effort; may be 0 if empty).
    """
    client = _get_client()
    name = _collection_name("persona", default=PERSONA_COLLECTION)

    # Best option: delete + recreate the collection (fast, reliable).
    try:
        col = _get_collection(name)
        before = col.count()
        client.delete_collection(name)  # clears only this collection
        _get_collection(name)  # recreate so future calls work
        return before
    except Exception:
        # Fallback: delete by ids (works even if delete_collection isn't available)
        col = _get_collection(name)
        before = col.count()
        if before <= 0:
            return 0
...
def recall_semantic(query: str, top_k: int = 6) -> List[str]:
    ...
    Uses Orion's explicit embedding path so semantic recall stays aligned with
    ingestion and does not depend on Chroma's query_texts embedding behavior.
    """
    col = _semantic()
