...
from orion_cli.shared.paths import USER_DATA_DIR
...
def _abs_under_user_data(p: Path) -> Path:
    # If config gives a relative path, interpret it relative to USER_DATA_DIR
...
def ingest_persona_for_active_profile(
    plan: bool = typer.Option(
        False, "--plan", help="Print what would be ingested without writing to DB."
    ),
    replace: bool = typer.Option(
        False, "--replace", help="DANGEROUS: clear persona collection before ingest."
    ),
    confirm: str = typer.Option(
        "",
        "--confirm",
        help="Required when using --replace. Must equal REPLACE_PERSONA.",
    ),
):
    if plan and replace:
        raise typer.BadParameter("Use either --plan or --replace, not both.")
    if plan and (confirm or "").strip():
        raise typer.BadParameter(
            "You passed --confirm with --plan. --confirm is only used with --replace."
        )
    """
    ...
    confirm_norm = (confirm or "").strip()

    # --- Guard combinations ---
    if plan and replace:
        raise typer.BadParameter("Use either --plan or --replace, not both.")
    if plan and confirm_norm:
        raise typer.BadParameter("--confirm is only used with --replace (not --plan).")

    if confirm_norm and not replace:
        raise typer.BadParameter(
            "You passed --confirm without --replace. Did you mean to use --replace?"
        )

    # --- Read YAML before any destructive action ---
    ...
    if plan:
        # Count how many entries would actually be ingested (same rules as the ingest loop)
        would_add = 0
        for doc in docs:
        ...
        typer.echo("=== Persona Plan (dry run) ===")
        typer.echo(f"Profile:      {cfg.profile!r}")
        typer.echo(f"Persona file: {persona_path}")
        typer.echo(f"Docs total:   {len(docs)}")
        typer.echo(f"Would add:    {would_add}")
        return
